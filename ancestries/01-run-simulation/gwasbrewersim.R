...
### simulate data for EAS
set.seed(1234)
nrep = 100
G <- matrix(0, nrow = nrep, ncol = nrep) # simulating 100 traits such that there is no causal relationship between them (distinct eQTL studies)
# and they have identical effect sizes, same CV, and all in EAS ancestry
effects <- matrix(0, nrow = length(ref_leg_EAS[do_sim==T,id]) , ncol = nrep)
cvind <- which(ref_leg_EAS[do_sim==T,id] == cvsall$id[i])
effects[cvind, ] <- beta
effects <- as.matrix(effects)
eassim <- sim_mv_determined(N = rep(easn,nrep), 
                            direct_SNP_effects_joint = effects, 
                            geno_scale = "allele", 
                            pheno_sd = 1, 
                            G=G, 
                            est_s = T, 
                            R_LD = LD_EAS, 
                            af = ref_leg_EAS$EAS[ref_leg_EAS$do_sim==T])

easzsim <- eassim$beta_hat / eassim$s_estimate
easpsim <- 2*pnorm(-abs(easzsim))
...
for (j in 1:nrep) {
  ...
  EAS_sumstats$BETA <- as.vector(eassim$beta_hat[,j])
  EAS_sumstats$se <- as.vector(eassim$s_estimate[,j])
  EAS_sumstats$stat <- as.vector(easzsim[,j])
  EAS_sumstats$p <- as.vector(easpsim[,j])
  
  fwrite(AFR_sumstats, paste("AFR_eqtl", j, ".txt", sep=""), sep = "\t")
  fwrite(EUR_sumstats, paste("EUR_eqtl", j, ".txt", sep=""), sep = "\t")
  fwrite(EAS_sumstats, paste("EAS_eqtl", j, ".txt", sep=""), sep = "\t")
}
