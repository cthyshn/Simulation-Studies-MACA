...
for (i in vec) {
  ...
  locilevel_long <- melt(results, id.vars = "setting", measure.vars = c("cs_ll", "cm_ll", "es_ll", "em_ll"), 
                         variable.name = "Method", value.name = "locilevel")
  locilevel_long$Method <- factor(locilevel_long$Method, levels = c("cs_ll", "cm_ll", "es_ll", "em_ll"),
  ...
  locilevel_plot <- ggplot(na.omit(locilevel_long), aes(x=setting, y=locilevel, fill = Method)) + geom_boxplot(outlier.size = 0.75) +
    labs(y="lCLPP", fill = "Method", x = "EUR:AFR") + theme_classic() +
  ...
  plots[[i]] <- locilevel_plot
  
}

legend <- get_legend(locilevel_plot + theme(legend.position = "bottom", legend.text = element_text(size = 12), legend.title = element_text(size = 12)))
