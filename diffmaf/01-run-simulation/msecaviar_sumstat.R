setwd(paste0("../results/", split, "/", cvsall$id[i]))
...
keep <- intersect(afrss$snp, eurss$snp) 
...
for (j in 1:100) {
  ...
  content <- c(paste0("AFR_gwas", j, ".zscores"), paste0("EUR_gwas", j, ".zscores"))
...
for (j in 1:100) {
  ...
  content <- c(paste0("AFR_eqtl", j, ".zscores"), paste0("EUR_eqtl", j, ".zscores"))
...
writeLines(c("AFR.ld", "EUR.ld"), "MseCAVIAR/ldfiles.txt")
#}
