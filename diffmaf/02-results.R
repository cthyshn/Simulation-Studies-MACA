...
for (j in 1:nrow(cvsall)) {
  cv <- cvsall$id[j]
  setwd(paste0("results/", split, "/", cv))
  for (i in 1:100) {
    res$iter[i] <- i
    sgwas <- fread(paste0("coloc+SuSiEx/gwas", i, ".SuSiEx.EUR.AFR.output.cs95.snp"))
    seqtl <- fread(paste0("coloc+SuSiEx/eqtl", i, ".SuSiEx.EUR.AFR.output.cs95.snp"))
    ...
    mgwas <- fread(paste0("MseCAVIAR/gwas", i, "_", "eqtl", i, ".msecaviar.EUR.AFR_1_post.txt"))
    meqtl <- fread(paste0("MseCAVIAR/gwas", i, "_", "eqtl", i, ".msecaviar.EUR.AFR_2_post.txt"))
...
setwd("../../results")
write.csv(allres, "80:20allres2.csv")
